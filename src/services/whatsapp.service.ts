...
export class WhatsappService {
  ...
  private async checkInitialConnectionStatus(instance: WhatsappInstance): Promise<void> {
    try {
      const connectionState = await firstValueFrom(
        this.evolutionApi.getConnectionState(instance.instanceApiKey, instance.instanceName)
      );
  ...
  async connect(): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(this.evolutionApi.connect(instance.instanceApiKey, instance.instanceName));
  ...
  async disconnect(): Promise<void> {
    ...
    this.stopPolling();
    try {
      await firstValueFrom(this.evolutionApi.logout(instance.instanceApiKey, instance.instanceName));
      this.status.set('disconnected');
    } catch (e) {
  ...
  private startPolling(): void {
    ...
    interval(this.pollingInterval).pipe(
      ...
      switchMap(() => this.evolutionApi.getConnectionState(instance.instanceApiKey, instance.instanceName))
    ).subscribe(state => {
